...
import { compactObject, requireDestructiveConfirmation } from '../utils.js';
...
export const suppressionsToolHandlers: Record<string, ToolHandler> = {
  ...
  list_bounces: async (service: SendGridService, args: Record<string, any>) => {
    const bounces = await service.listBounces(
      compactObject({
        start_time: args.start_time,
        end_time: args.end_time
      })
    );
    return { content: [{ type: 'text', text: JSON.stringify(bounces, null, 2) }] };
  },
  ...
  list_invalid_emails: async (service: SendGridService, args: Record<string, any>) => {
    const invalidEmails = await service.listInvalidEmails(
      compactObject({
        start_time: args.start_time,
        end_time: args.end_time
      })
    );
    return { content: [{ type: 'text', text: JSON.stringify(invalidEmails, null, 2) }] };
  },
  list_spam_reports: async (service: SendGridService, args: Record<string, any>) => {
    const spamReports = await service.listSpamReports(
      compactObject({
        start_time: args.start_time,
        end_time: args.end_time
      })
    );
    return { content: [{ type: 'text', text: JSON.stringify(spamReports, null, 2) }] };
  }
};
