...
export interface SendGridListResponse<T> {
  result: T[];
  _metadata?: Record<string, any>;
  [key: string]: any;
}

export interface SendGridDesignListParams {
  page_size?: number;
  page_token?: string;
  summary?: boolean;
}

export interface SendGridDesignInput {
  name: string;
  subject?: string;
  html_content?: string;
  plain_content?: string;
...
export interface SendGridTemplateVersionInput {
  name: string;
  subject: string;
  html_content: string;
  plain_content: string;
...
export type SendGridStats = Array<{
  date: string;
  stats: Array<{
    metrics: {
      opens: number;
